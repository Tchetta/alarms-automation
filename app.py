...
st.write("Upload your fresh dashboard export alongside your PM planning and Parent Sites reference files to build your formatted report.")

# 1. FILE UPLOAD INTERFACE
uploaded_alarm = st.file_uploader("1. Upload Power Alarm Export (.xlsx or .csv)", type=["xlsx", "csv"])
uploaded_pm = st.file_uploader("2. Upload PM Plan Reference (Q2_PM_Plan...)", type=["xlsx"])
...
if uploaded_alarm and uploaded_pm and uploaded_parent:
    st.success("All three files staged successfully.")
    ...
    if st.button("🚀 Generate Formatted Alarm Report"):
        with st.spinner("Processing network structures and applying styling configurations..."):
            try:
                ...
                # Ensure consistent string structures for key lookups, then convert Site IDs to numeric safely
                ...
                # Filter down to the essential columns you mentioned keeping
                ...
                # Prepare dictionaries for lookup optimizations (VLOOKUP equivalents)
                # PM Plan lookup: Key = Col A (Site ID), Value = Col E (Power Owner, index 4)
                pm_dict = df_pm.dropna(subset=[df_pm.columns[0]]).set_index(df_pm.columns[0])[df_pm.columns[4]].to_dict()
                
                # Parent sites lookups: Key = Col A, Values = Col F (Num Children, index 5) & Col H (Power Owner alternative, index 7)
                ...
                # Map attributes using our dictionaries
                df_clean['Power Owner'] = df_clean['Site ID'].map(pm_dict)
                # Fallback to parent file power owner if missing from PM planning
                df_clean['Power Owner'] = df_clean['Power Owner'].fillna(df_clean['Site ID'].map(parent_owner_dict)).fillna("Unknown")
                ...
                df_clean['Prediction'] = temp_children.apply(
                    lambda x: f"{int(x)} sites will be affected" if pd.notna(x) and isinstance(x, (int, float, round()._class_)) else ""
                )
                ...
                wb = load_workbook(io.BytesIO()) # Instantiates clean workbook context
                ws = wb.active
                ...
                time_string = rounded_time.strftime("%HH%M") # e.g., 16H30
